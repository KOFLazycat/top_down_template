...
func _ready()->void:
	if move_node == null:
		Log.entry("未配置目标节点（move_node），无法执行重叠修正", LogManager.LogLevel.ERROR)
		return
...
func _fix()->void:
	if !is_colliding():  # 若未检测到碰撞，直接返回
		Log.entry("未检测到碰撞，无需修正", LogManager.LogLevel.INFO)
		return
	...
	Log.entry("修正完成，移动距离：%s" % _solid_distance, LogManager.LogLevel.INFO)
