...
func _ready()-> void:
	if astargrid_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "TileAstargridObstacle: AstarGridResource 未正确设置！", LogManager.LogLevel.ERROR)
		return
...
func setup_obstacles()->void:
	...
	for i:int in tiles.size():
		...
		if _offset_list == null || !(_offset_list is PackedVector2Array):
			Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "TileAstargridObstacle: 单元格 %s 的自定义数据层 {data_name} 无有效偏移量，跳过" % _tile_pos, LogManager.LogLevel.INFO)
			continue
