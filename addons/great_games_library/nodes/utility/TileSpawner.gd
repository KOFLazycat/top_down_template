...
func _on_parent_updated()->void:
	...
	for i:int in tiles.size():
		...
		if _tile_data == null:
			#Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "TileSpawner: 单元格 %s 未设置 TileData，跳过实例生成" % _tile_pos, LogManager.LogLevel.ERROR)
		...
		if not _scene_cache.has(_file_path):
			...
			if not ResourceLoader.exists(_file_path):
				Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "TileSpawner: 场景路径 %s 不存在，跳过实例生成" % _file_path, LogManager.LogLevel.ERROR)
				continue
			
			var _scene:PackedScene = load(_file_path)
			if not _scene.is_class("PackedScene"):
				Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "TileSpawner: %s 不是有效的 PackedScene 资源，跳过实例生成" % _file_path, LogManager.LogLevel.ERROR)
				continue
