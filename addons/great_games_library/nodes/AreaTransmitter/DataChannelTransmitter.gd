...
func send(receiver:AreaReceiver2D)->void:
	if receiver == null || !receiver.is_inside_tree():
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "DataChannelTransmitter: 接收方无效（已销毁或未加入场景树）", LogManager.LogLevel.ERROR)
		return
	
	if !enabled:  # 若传输被禁用，直接返回
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "DataChannelTransmitter: 传输被禁用，直接返回", LogManager.LogLevel.INFO)
		return
	
	# 确保传输资源已配置（调试模式生效）
	if transmission_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "DataChannelTransmitter: 传输资源（transmission_resource）未配置，无法发送", LogManager.LogLevel.ERROR)
		return
...
func test_receiver(receiver:AreaReceiver2D)->void:
	if receiver is not AreaReceiver2D:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "DataChannelTransmitter: receiver 类型不正确", LogManager.LogLevel.ERROR)
		return
