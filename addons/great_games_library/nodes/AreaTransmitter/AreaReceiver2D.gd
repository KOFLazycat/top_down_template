...
func receive(transmission_resource:TransmissionResource)->void:
	...
	if transmission_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AreaReceiver2D: 传输资源为空，无法处理接收请求", LogManager.LogLevel.ERROR)
		return
	...
	if callbacks.has(transmission_name):
		...
		if callback.is_valid():  # 校验回调有效性（避免无效 Callable）
			...
			Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AreaReceiver2D: 回调函数无效，传输名称：%s" % transmission_name, LogManager.LogLevel.ERROR)
			transmission_resource.failed()
		return
	
	# 未找到匹配的回调时标记传输失败
	Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AreaReceiver2D: 未找到传输名称对应的回调：%s" % transmission_name, LogManager.LogLevel.WARNING)
	transmission_resource.failed()
...
func add_receiver(transmission_name:StringName, callback:Callable)->void:
	if transmission_name.is_empty():
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AreaReceiver2D: 传输名称为空，无法注册回调", LogManager.LogLevel.ERROR)
		return
	if callback == null || !callback.is_valid():
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AreaReceiver2D: 无效的回调函数，传输名称：%s" % transmission_name, LogManager.LogLevel.ERROR)
		return
	
	callbacks[transmission_name] = callback  # 添加回调到字典
	Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AreaReceiver2D: 注册传输回调成功：%s" % transmission_name, LogManager.LogLevel.INFO)
...
func remove_receiver(transmission_name:StringName)->void:
	if callbacks.has(transmission_name):
		...
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AreaReceiver2D: 移除传输回调：%s" % transmission_name, LogManager.LogLevel.INFO)
