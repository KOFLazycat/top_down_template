...
func _ready()->void:
	...
	for item:ResourceNodeItem in list:
		...
		if item.resource_name.is_empty():
			Log.entry("resource_name 为空，无法作为字典键", LogManager.LogLevel.ERROR)
			return
		# 资源引用非空（避免空值错误）
		if item.resource == null:
			Log.entry("resource 未配置，资源条目无效", LogManager.LogLevel.ERROR)
			return
		# 资源名称重复
		if dictionary.has(item.resource_name):
			Log.entry("资源名称重复，%s 重复加入字典" % item.resource_name, LogManager.LogLevel.WARNING)
		# 复制资源条目（避免修改原始配置）
...
func add_resource(item:ResourceNodeItem)->void:
	...
	# 资源名称重复
	if dictionary.has(item.resource_name):
		Log.entry("add_resource: 资源名称重复，%s 重复加入字典" % item.resource_name, LogManager.LogLevel.WARNING)
