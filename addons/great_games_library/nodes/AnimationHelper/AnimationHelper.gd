...
func play(animation:StringName, reset:bool = false)->void:
	...
	if animation_player == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AnimationHelper: 动画播放器（animation_player）未配置，无法播放动画", LogManager.LogLevel.ERROR)
		return
	if !animation_player.has_animation(animation):
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AnimationHelper: 动画播放器缺少目标动画：%s" % animation, LogManager.LogLevel.ERROR)
		return
