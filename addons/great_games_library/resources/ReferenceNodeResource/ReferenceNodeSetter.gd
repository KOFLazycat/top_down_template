...
func _ready()->void:
	...
	if !reference_resource_path.is_empty():
		...
		if loaded_resource is ReferenceNodeResource and loaded_resource != null:
			...
			Log.entry("资源路径 %s 加载失败或类型错误（需为 ReferenceNodeResource）" % reference_resource_path, LogManager.LogLevel.ERROR)
			return  # 提前返回避免后续逻辑出错

	if reference_node == null || !reference_node.is_inside_tree():
		Log.entry("reference_node 未配置或已失效，跳过引用绑定", LogManager.LogLevel.ERROR)
		return
