...
func callback(value_list:Array)->void:
	...
	if callable.is_null():
		Log.entry("回调函数未设置，无法执行", LogManager.LogLevel.ERROR)
		return
...
func command(method:StringName, value_list:Array)->void:
	...
	if node == null || !node.is_instance_valid():
		Log.entry("节点引用无效（未设置或已销毁）", LogManager.LogLevel.ERROR)
		return
	if !node.has_method(method):
		Log.entry("节点 %s 未实现方法 %s" % [node.name, method], LogManager.LogLevel.ERROR)
		return
