...
func play()->void:
	...
	if enabled_settings == null:
		Log.entry("未配置全局启用设置（enabled_settings）", LogManager.LogLevel.ERROR)
		return
	...
	if camera_reference == null || camera_reference.node == null:
		Log.entry("相机引用无效（未设置或已销毁）", LogManager.LogLevel.ERROR)
		return
...
func sample(t:float)->void:
	if camera_reference.node == null:
		Log.entry("相机引用无效已销毁", LogManager.LogLevel.ERROR)
