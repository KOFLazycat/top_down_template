...
func _ready()->void:
	...
	if player_reference == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "PlayerSpawner: 玩家引用资源未配置", LogManager.LogLevel.ERROR)
		return
	if player_instance_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "PlayerSpawner: 玩家实例资源未配置", LogManager.LogLevel.ERROR)
		return
...
func on_player_scene_entry()->void:
	...
	if scene_transition_resource.entry_match == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "PlayerSpawner: 场景过渡入口节点未配置，玩家位置同步失败", LogManager.LogLevel.ERROR)
		return
	if !scene_transition_resource.entry_match.is_inside_tree():
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "PlayerSpawner: 入口节点不在场景树中，使用默认生成位置", LogManager.LogLevel.ERROR)
		return
	if scene_transition_resource.entry_match.is_queued_for_deletion():
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "PlayerSpawner: 入口节点已入队删除", LogManager.LogLevel.ERROR)
		return
	...
	if player_instance_resource.parent_reference_resource.node == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "PlayerSpawner: 玩家父节点未配置", LogManager.LogLevel.ERROR)
		return
	# 将玩家节点添加到实例资源指定的父节点中
