...
func _ready()->void:
	if resource_node == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "SpriteFlip: 资源节点（resource_node）未配置", LogManager.LogLevel.ERROR)
		return
	# 从资源节点获取输入资源（确保非空）
	input_resource = resource_node.get_resource("input")
	if input_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "SpriteFlip: 输入资源（input）未配置", LogManager.LogLevel.ERROR)
		return
	
	if flip_node == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "SpriteFlip: 翻转节点（flip_node）未配置，无法执行翻转逻辑", LogManager.LogLevel.ERROR)
		return
