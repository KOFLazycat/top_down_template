...
func _ready()->void:
	if inventory_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "WeaponInventory: 未配置物品集合资源（inventory_resource）", LogManager.LogLevel.ERROR)
		return
	...
	for i:int in inventory_resource.max_items:
		...
		if !_slot.has_node(prefab_icon_nodepath):
			Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "WeaponInventory: 槽位预制体中未找到图标节点：%s" % prefab_icon_nodepath, LogManager.LogLevel.ERROR)
			continue
		# 获取槽位内的图标节点并保存引用
