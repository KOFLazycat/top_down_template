...
func _ready()->void:
	...
	if not animation_player or not boot_preloader:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "BootScreenManager: 未绑定 animation_player 或 boot_preloader，启动流程终止！", LogManager.LogLevel.ERROR)
		return

	# 检查动画是否存在（使用常量字符串）
	if not animation_player.has_animation(ANIM_IDLE):
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "BootScreenManager: 动画播放器缺少 %s 动画！" % ANIM_IDLE, LogManager.LogLevel.ERROR)
		return
...
func _transition_out()->void:
	...
	if not animation_player.has_animation(ANIM_TRANSITION_OUT):
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "BootScreenManager: 动画播放器缺少 %s 动画！" % ANIM_TRANSITION_OUT, LogManager.LogLevel.ERROR)
		return
