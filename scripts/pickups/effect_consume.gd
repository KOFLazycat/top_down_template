...
func _ready()->void:
	...
	if data_transmitter == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "EffectConsume: 数据传输器（data_transmitter）未配置，节点：%s" % name, LogManager.LogLevel.ERROR)
		return
	if animation_player == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "EffectConsume: 动画播放器（animation_player）未配置，将无法播放消耗动画", LogManager.LogLevel.ERROR)
		return
	if sounds_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "EffectConsume: 音效资源（sounds_resource）未配置，将无法播放消耗音效", LogManager.LogLevel.ERROR)
		return
...
func _on_success()->void:
	...
	# 播放消耗音效（自动管理音效生命周期）
	if sounds_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "EffectConsume: 音效资源未加载或路径错误，节点：%s" % name, LogManager.LogLevel.ERROR)
		return
	else:
