...
func _ready() -> void:
	if item_resource == null:
		Log.entry("物品资源（item_resource）未配置，节点：%s" % name, LogManager.LogLevel.ERROR)
		return
	if icon_sprite == null:
		Log.entry("图标精灵（icon_sprite）未配置，节点：%s" % name, LogManager.LogLevel.ERROR)
		return
	if data_transmitter == null:
		Log.entry("数据传输器（data_transmitter）未配置，节点：%s" % name, LogManager.LogLevel.ERROR)
		return
	if sound_resource == null:
		Log.entry("音效资源（sound_resource）未配置，拾取时无音效", LogManager.LogLevel.ERROR)
		return
...
func prepare_remove()->void:
	if !item_resource.unlocked:
		Log.entry("物品未解锁，无法拾取，节点：%s" % name, LogManager.LogLevel.ERROR)
		return
