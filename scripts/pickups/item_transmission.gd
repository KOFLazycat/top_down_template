...
func process(resource_node:ResourceNode)->void:
	if item_resource == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "ItemTransmission: 物品资源（item_resource）未配置，传输失败", LogManager.LogLevel.ERROR)
		failed()
		return
	if resource_node == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "ItemTransmission: 资源节点（resource_node）为空，传输失败", LogManager.LogLevel.ERROR)
		failed()
		return
	...
	match item_resource.type:
		...
		_:
			Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "ItemTransmission: 未支持的物品类型：%s" % item_resource.type, LogManager.LogLevel.ERROR)
			failed()  # 默认标记为传输失败
...
func _weapon(resource_node:ResourceNode)->void:
	...
	if _weapon_inventory == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "ItemTransmission: 资源节点缺少武器背包（weapons），传输失败", LogManager.LogLevel.ERROR)
		failed()
		return
	
	# 检查武器背包是否已满（容量超过最大值）
	if _weapon_inventory.list.size() >= _weapon_inventory.max_items:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "ItemTransmission: 传输失败：背包（%s）已满（容量：%d/%d）" % [
