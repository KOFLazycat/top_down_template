...
func _ready() -> void:
	...
	|| player_reference == null || sound_collect == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "CollectingPoint: 关键导出变量未配置，节点：%s" % name, LogManager.LogLevel.ERROR)
		return
...
func _on_body_enter(_body:Node2D)->void:
	if player_reference.node == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "CollectingPoint: 玩家引用为空，无法执行收集逻辑", LogManager.LogLevel.ERROR)
		return
...
func _tween_target_position(t:float, from:Vector2)->void:
	if player_reference.node == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "CollectingPoint: 玩家引用为空，tween动画失效", LogManager.LogLevel.ERROR)
		return
