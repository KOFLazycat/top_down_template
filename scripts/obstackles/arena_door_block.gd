...
func update()->void:
	if position_node == null or animation_player == null:
		Log.entry("position_node或animation_player未设置，无法更新门状态", LogManager.LogLevel.ERROR)
		return
	
	if not animation_player.has_animation(animation_on) or not animation_player.has_animation(animation_off):
		Log.entry("动画名称无效：animation_on=%s, animation_off=%s" % [animation_on, animation_off], LogManager.LogLevel.ERROR)
		return
	
	# 检查A*网格资源是否有效
	if astargrid_resource.value == null:
		Log.entry("A*网格资源无效", LogManager.LogLevel.ERROR)
		return
	...
	# 如果状态没有变化，则不执行任何操作
	if _new_state == state:
		Log.entry("状态没有变化，不执行任何操作", LogManager.LogLevel.INFO)
		return
