...
func setup(
	...
	if sprite == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AfterImageVFX: 残像精灵（sprite）未配置，setup 失败", LogManager.LogLevel.ERROR)
		return
	# 其他参数校验（示例：纹理不能为空）
	if texture == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AfterImageVFX: 残像纹理（texture）为空，可能导致显示异常", LogManager.LogLevel.ERROR)
		return
...
func _ready() -> void:
	...
	if !animation_player.has_animation(animation):
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "AfterImageVFX: 动画（%s）不存在，残像无法播放" % animation, LogManager.LogLevel.ERROR)
