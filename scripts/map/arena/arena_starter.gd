...
func _ready()->void:
	if fight_mode == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "ArenaStarter: 战斗模式资源未配置", LogManager.LogLevel.ERROR)
		return
	if area == null:
		Log.entry("[" + get_script().resource_path.get_file().get_basename() + ".gd] " + "[" + str(get_stack()[0]["line"] if get_stack()[0].size() > 0 else -1) + "] " + "ArenaStarter: 触发区域未配置", LogManager.LogLevel.ERROR)
		return
